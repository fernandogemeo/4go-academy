...
export default function BlackPlanejamentoGuz() {
  const [activeWeek, setActiveWeek] = useState<number | null>(null)

  const [checked, setChecked] = useState<Record<string, boolean>>(() => {
    try { return JSON.parse(localStorage.getItem("bf-guz-checklist-2026") || "{}") }
    catch { return {} }
  })

  const [config, setConfig] = useState(() => {
    try { return { ...defaultConfig, ...JSON.parse(localStorage.getItem("bf-guz-config-2026") || "{}") } }
    catch { return { ...defaultConfig } }
  })

  const [entregaveis, setEntregaveis] = useState<string[]>(() => {
    try {
      const stored = JSON.parse(localStorage.getItem("bf-guz-entregaveis-2026") || "null")
  ...
  const [narrativa, setNarrativa] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem("bf-guz-narrativa-2026") || "null") || defaultNarrativa }
    catch { return [...defaultNarrativa] }
  })

  useEffect(() => {
    localStorage.setItem("bf-guz-checklist-2026", JSON.stringify(checked))
  }, [checked])

  useEffect(() => {
    localStorage.setItem("bf-guz-config-2026", JSON.stringify(config))
  }, [config])

  useEffect(() => {
    localStorage.setItem("bf-guz-entregaveis-2026", JSON.stringify(entregaveis))
  }, [entregaveis])

  useEffect(() => {
    localStorage.setItem("bf-guz-narrativa-2026", JSON.stringify(narrativa))
  }, [narrativa])
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#eee]">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <span className="font-bold text-[#0a0a0a] hidden sm:block">GUZ</span>
          </div>
      ...
      <footer className="border-t border-[#eee] py-8 px-6 bg-[#fafafa]">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[#999] text-sm">GUZ · Black Friday 2026</span>
          </div>
          <div className="flex items-center gap-3">
